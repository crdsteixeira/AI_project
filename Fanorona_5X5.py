...
# Fanorona on 5X5 grid played by one human player and one AI player.
...
GRID_SIZE = 60  # width & height of each position on the grid, in pixels
GRID_WIDTH = 5  # how many columns of grid on the game board
GRID_HEIGHT = 5  # how many rows of spaces on the game board
...
def main():
    pygame.init()
    pygame.display.set_caption('Fanorona 5X5 by Guang XIONG')
    ...
    while True:
        if run_fanorona_5X5() == False:
            break


def run_fanorona_5X5():
    """Plays a round of game Fanorona each time this function is called.
    """
    main_grid = get_new_grid_5X5()  # 2-dimensional array to stores information of all tokens
    draw_grid(main_grid)
    global difficulty
...
def translate_grid_to_pixel_coord(xxx_todo_changeme):
    (grid_column, grid_row) = xxx_todo_changeme
    return grid_column * int(WINDOW_HEIGHT*0.125) + int(WINDOW_HEIGHT*0.25),\
           grid_row * int(WINDOW_WIDTH*0.125) + int(WINDOW_WIDTH*0.25)


def get_grid_clicked(xxx_todo_changeme1):
    """ Return a tuple of two integers of the grid space coordinates where
    the mouse was clicked. (Or returns None not in any space.)
    """
    (mouse_x, mouse_y) = xxx_todo_changeme1
    for column in range(GRID_WIDTH):
        for row in range(GRID_HEIGHT):
...
def make_move(token_color, grid, xxx_todo_changeme2, xxx_todo_changeme3, prompt_bi_direct=False):
...
def check_for_draw(AI_state, turn):
    """
      use pattern matching for draw or timer
    """
    pass
...
def show_game_results(winner_str, loser_str, draw=False):
    ...
    global depth_of_game_tree
    print(("\n\nfinally,  level is, depth in termination ", depth_of_game_tree, '\n\n', total_node_generated))
    if draw:
