...
def terminal_test(AI_state):
    ...
    for column in range(my_board.GRID_COLS):
        for row in range(my_board.GRID_ROWS):
...
def translate_grid_to_pixel_coord(xxx_todo_changeme):
    ...
    if cols == 3:
        x = grid_column * int(WINDOW_HEIGHT*0.25) + int(WINDOW_HEIGHT*0.25),\
...
def get_grid_clicked(xxx_todo_changeme1):
    ...
    for column in range(my_board.GRID_COLS):
        for row in range(my_board.GRID_ROWS):
            ...
            if (mouse_x > center_x - int(my_board.GRID_SIZE*0.5) and
                mouse_x < center_x + int(my_board.GRID_SIZE*0.5) and
                mouse_y > center_y - int(my_board.GRID_SIZE*0.5) and
                mouse_y < center_y + int(my_board.GRID_SIZE*0.5)):
                return (column, row)
    return None
